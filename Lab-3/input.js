...
for (let item of divToDrag) {
  ...
  item.addEventListener("touchstart", (event) => {
    ...
    if (difference < 300) {
      ...
      itemSize.width = parseInt(window.getComputedStyle(holderItem).width);
      itemSize.height = parseInt(window.getComputedStyle(holderItem).height);
...
workSpace.addEventListener("touchmove", (event) => {
  ...
  if (isDragging && holderItem) {
    ...
    if (
      itemSize.newSquare >= itemSize.square * 0.5 &&
      itemSize.newSquare <= itemSize.square * 2
    ) {
      itemSize.width += i;
      itemSize.height += i;
